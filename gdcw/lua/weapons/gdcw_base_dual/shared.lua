...
function SWEP:IronSight()
	...
	if self.Owner:KeyPressed(IN_SPEED) || self.Owner:KeyPressed(IN_USE) then	// If you run then
		...
		if self.Owner:KeyPressed(IN_ATTACK2) and !self.Owner:KeyDown(IN_USE) and !self.Owner:KeyDown(IN_SPEED) and !self.Owner:KeyDown(IN_DUCK) then
			self.Owner:SetFOV(self.Secondary.IronFOV, 0.15 )
...
function SWEP:DrawHUD()
	...
	if  self.Owner:KeyDown(IN_ATTACK2) and (self:GetIronsights() == true) and (!self.Owner:KeyDown(IN_SPEED) and !self.Owner:KeyDown(IN_USE)) and self.Owner:KeyDown(IN_DUCK) then
...
function SWEP:AdjustMouseSensitivity()
	...
	if self.Owner:KeyDown(IN_ATTACK2) and self.Owner:KeyDown(IN_DUCK) then
