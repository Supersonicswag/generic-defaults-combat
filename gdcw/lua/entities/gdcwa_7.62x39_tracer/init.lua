...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create ("gdcwa_7.62x39_tracer")
	ent:SetPos (SpawnPos)
	ent:Spawn()
	ent:Activate()
	return ent
...
self.penetrate = 15
self.flightvector = self.Entity:GetUp() * ((715*39.37)/66)             	-- Velocity in m/s, inches to meters conversion, ticks per second.FIRST NUMMER = SPEED
...
function ENT:Think()
		...
		FireTrail:SetKeyValue("rendercolor","150 250 100")
	...
	local tr = util.TraceLine( trace )
				...
				local dmginfo = DamageInfo()
					dmginfo:SetDamage( math.Rand(70,110) ) 	-- 1 or 2 hits for a kill
	...
	self.flightvector = self.flightvector - self.flightvector/120 + Vector(math.Rand(-0.1,0.1), math.Rand(-0.1,0.1),math.Rand(-0.1,0.1)) + Vector(0,0,-0.06)
