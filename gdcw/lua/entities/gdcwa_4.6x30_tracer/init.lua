...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create ("gdcwa_4.6x33_tracer")
	ent:SetPos (SpawnPos)
	ent:Spawn()
	ent:Activate()
	return ent
...
self.penetrate = 7
self.flightvector = self.Entity:GetUp() * ((725*39.37)/66)             	-- Velocity in m/s, inches to meters conversion, ticks per second.FIRST NUMMER = SPEED
...
function ENT:Think()
		...
		FireTrail:SetKeyValue("startwidth","17")
	...
	local tr = util.TraceLine( trace )
				...
				return true
					...
					util.BlastDamage(self.Entity, self.Owner, tr.HitPos, 30, 5)
				...
				local dmginfo = DamageInfo()
					dmginfo:SetDamage( 25 ) 			--1 to 2 hits for a kill
