...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create ("gdcwa_7.62x51_lowvel")
	ent:SetPos (SpawnPos)
	ent:Spawn()
	ent:Activate()
	return ent
...
self.penetrate = 20
self.flightvector = self.Entity:GetUp() * ((730*39.37)/66)             	-- Velocity in m/s, inches to meters conversion, ticks per second.FIRST NUMMER = SPEED
...
function ENT:Think()
	...
	local tr = util.TraceLine( trace )
				...
				return true
					...
					util.BlastDamage(self.Entity, self.Owner, tr.HitPos, 80, 10)
					local effectdata = EffectData()
					effectdata:SetOrigin(tr.HitPos)
					effectdata:SetNormal(tr.HitNormal)
					effectdata:SetScale(1.25)
					effectdata:SetRadius(1.25)
			...
			if tr.Hit and tr.Entity:IsPlayer() || tr.Entity:IsNPC() then
				local dmginfo = DamageInfo()
					dmginfo:SetDamage( math.Rand(60,110) ) 	-- 1 to 2 hits for a kill
	...
	self.flightvector = self.flightvector - self.flightvector/150 + Vector(math.Rand(-0.05,0.05), math.Rand(-0.05,0.05),math.Rand(-0.05,0.05)) + Vector(0,0,-0.06)
