...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create ("gdcwa_357_490")
	ent:SetPos (SpawnPos)
	ent:Spawn()
	ent:Activate()
	return ent
...
self.penetrate = 10
self.flightvector = self.Entity:GetUp() * ((490*39.37)/66)             	-- Velocity in m/s, inches to meters conversion, ticks per second.FIRST NUMMER = SPEED
...
function ENT:Think()
	...
	local tr = util.TraceLine( trace )
				...
				local dmginfo = DamageInfo()
					dmginfo:SetDamage( math.Rand(35,60) ) 	--1 or 2 hits for a kill
