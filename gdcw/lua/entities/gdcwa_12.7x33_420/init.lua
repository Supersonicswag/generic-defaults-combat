...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create ("gdcwa_12.7x33_420")
	ent:SetPos (SpawnPos)
	ent:Spawn()
	ent:Activate()
	return ent
...
self.penetrate = 10
self.flightvector = self.Entity:GetUp() * ((420*39.37)/66)             	-- Velocity in m/s, inches to meters conversion, ticks per second.FIRST NUMMER = SPEED
...
function ENT:Think()
	...
	local tr = util.TraceLine( trace )
				...
				local dmginfo = DamageInfo()
					dmginfo:SetDamage( math.Rand(45,60) ) 	--1 or 2 hits for a kill
	...
	self.flightvector = self.flightvector - self.flightvector/200 + Vector(math.Rand(-0.2,0.2), math.Rand(-0.2,0.2),math.Rand(-0.2,0.2)) + Vector(0,0,-0.06)
