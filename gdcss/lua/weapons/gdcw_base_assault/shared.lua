...
function SWEP:FireRocket()
	...
	if self.Owner:KeyPressed(IN_ATTACK1) then
	Aim = self.Owner:EyeAngles() print("BAA") else
	Aim = self.Owner:EyeAngles()-self.Anglo
	end
	else 
	Aim = self.Owner:EyeAngles()+Angle(math.Rand(-1,1), math.Rand(-1,1), 0)
	end
	if !self.Owner:IsNPC() then
	...
	if SERVER then
		...
		bullet:SetAngles(Aim+Angle(90,0,0))
		bullet:SetPos(pos)
		...
		if !self.Owner:IsNPC() then
		self.Anglo = Angle(math.Rand(-self.Primary.KickDown,self.Primary.KickUp), math.Rand(-self.Primary.KickHorizontal,self.Primary.KickHorizontal), 0)
		self.Owner:ViewPunch(self.Anglo)
		end
end
