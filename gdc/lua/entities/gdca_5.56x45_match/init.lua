...
function ENT:Think()
	...
	local tr = util.TraceLine( trace )
			...
			return true
				...
				if tr.Hit then
					util.BlastDamage(self.Entity, self.Entity, tr.HitPos, 70, 40)
					...
					util.ScreenShake(tr.HitPos, 10, 5, 1, 200 )
					util.Decal("ExplosiveGunshot", tr.HitPos + tr.HitNormal, tr.HitPos - tr.HitNormal)

					if tr.Entity:IsValid() then
					local attack =  gcombat.hcghit( tr.Entity, 20, 25, tr.HitPos, tr.HitPos) 	// Entity, Damage, Pierce
					end

					end

	local trace = {}
		trace.start = tr.HitPos + self.flightvector:GetNormalized() * 6
	...
	self.flightvector = self.flightvector + Vector(math.Rand(-0.1,0.1), math.Rand(-0.1,0.1),math.Rand(-0.1,0.1)) + Vector(0,0,-0.1)
